...
# ── OUTPUT DIR (matches HebbianGraph_ICA_SexClassification.py) ───────────────
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
N_ROIS      = 414      # Schaefer/Combined Atlas parcellation
TIMEPOINTS  = 490
SEED        = 26
TR          = 0.735    # seconds
...
ACTIVATION_THRESHOLD_PCT = 90.0   # slightly stricter than ICA (quadratic projection)
...
# TPM: N_ROIS=414 → upper triangle = 414*413/2 = 85,491 features; PCA reduces.
TPM_PCA_DIM = 50
FC_PCA_DIM  = 50

# ── SEX LABEL CONFIG (matches HebbianGraph_ICA_SexClassification.py) ─────────
# CSV with at least [EID, sex_31] columns (UK Biobank: 0=Female, 1=Male)
SEX_CSV_PATH   = "Atlas_Pearson_Full_Features.csv"
...
def build_records_from_files(
    ...
    """Match .csv Atlas files to sex labels via the first 7 characters of the filename."""
...
def screen_records(
    ...
    for rec in records:
        try:
            load_subject_timeseries(rec.csv_path, TIMEPOINTS, N_ROIS)
            retained.append(rec)
...
def bandpass_and_zscore(Z: np.ndarray, tr: float = TR) -> np.ndarray:
    """Z: (N_ROIS, N_TRs). Bandpass 0.01–0.10 Hz, then z-score per ROI."""
    nyq  = 0.5 / tr
...
def marchenko_pastur_lambda_max(Z: np.ndarray, sigma2: float = 1.0) -> float:
    """Upper Marchenko-Pastur bound: λ_max = σ²(1 + √(1/q))², q = N_TRs/N_ROIs."""
    N_rois, N_trs = Z.shape
    q = N_trs / N_rois
    return sigma2 * (1.0 + np.sqrt(1.0 / q)) ** 2

# ── STEP 3: DETECT ASSEMBLY PATTERNS ────────────────────────────────────────
...
def detect_assembly_patterns(
    ...
    """
    Lopes-dos-Santos (2013) / Almeida-Filho (2014) assembly detection:
    (a) ROI correlation matrix  (b) eigendecompose  (c) MP threshold
    (d) project onto top PCs    (e) FastICA → assembly weight patterns
...
# ── STEP 4: ASSEMBLY ACTIVITY TIME SERIES ────────────────────────────────────

def compute_assembly_activity(
    Z: np.ndarray,
    assembly_patterns: np.ndarray,
) -> np.ndarray:
    """AA_b = Z_b^T P Z_b, P = w w^T with diagonal zeroed (eq. 1, Lopes-dos-Santos 2013)."""
    ...
    for k, w in enumerate(assembly_patterns):
        ...
        PZ = P @ Z
        activity[k] = np.einsum("it,it->t", Z, PZ)
    return activity

# ── STEP 5: ACTIVATION DETECTION ─────────────────────────────────────────────
...
def extract_graph_features(
    ...
    l2 = sum(
        1 for u in G.nodes() for v in G.successors(u)
        if u < v and G.has_edge(v, u)
    )
    f["L2"] = l2 / norm
    l3 = sum(
        1 for u in G.nodes() for v in G.successors(u) if v != u
...
def compute_tpm(
    ...
    """
    Fixed-size N_ROIS × N_ROIS ROI co-activation TPM (upper triangle).
    414 ROIs → 85,491 features; reduced by PCA (TPM_PCA_DIM=50).
    """
...
# ── STEP 9: SUBJECT-LEVEL FEATURE EXTRACTION ─────────────────────────────────
...
def extract_subject_features(
    ...
    if verbose:
        log.info(f"  n_assemblies={n_assemblies} n_events={len(events)}")
...
def log_aggregate_metrics(all_metrics: List[Dict], label: str = ""):
    keys = [
        "auroc", "accuracy", "balanced_accuracy", "sensitivity_male",
...
def save_subject_features(subjects_bold, records, output_dir):
    ...
    log.info(
        f"  Assembly count — mean={np.mean(all_n_assemblies):.1f} "
        f"min={np.min(all_n_assemblies)} max={np.max(all_n_assemblies)}"
    )
    ...
    pd.concat([meta, pd.DataFrame(X_graph, columns=graph_names)], axis=1).to_csv(
        output_dir / "subject_graph_features.csv", index=False
    )

    tpm_cols = [f"tpm_{i}" for i in range(X_tpm.shape[1])]
    pd.concat([meta, pd.DataFrame(X_tpm, columns=tpm_cols)], axis=1).to_csv(
        output_dir / "subject_tpm_features.csv", index=False
    )

    fc_cols = [f"fc_{i}" for i in range(X_fc.shape[1])]
    pd.concat([meta, pd.DataFrame(X_fc, columns=fc_cols)], axis=1).to_csv(
        output_dir / "subject_fc_features.csv", index=False
    )
...
if __name__ == "__main__":
    ...
    # 2. Collect input .csv files (CLI args or current-dir glob)
    #    Swiss Army Knife passes all selected files as positional CLI arguments.
    if len(sys.argv) > 1:
        csv_files = sys.argv[1:]
    else:
        csv_files = sorted(glob.glob("*.csv", recursive=True))
        # Exclude the phenotype CSV itself if present in CWD
        csv_files = [f for f in csv_files if Path(f).name != Path(SEX_CSV_PATH).name]
    log.info(f"  {len(csv_files)} Atlas timeseries .csv files supplied.")

    # 3. Match files to sex labels via 7-digit EID prefix
    records, skipped = build_records_from_files(csv_files, sex_labels)
