...
# ---------------------------------------------------------------------------
# UK Biobank / Swiss Army Knife paths
# ---------------------------------------------------------------------------
# CSV that contains at least the columns [EID, sex_31].
# This is the Atlas-style phenotype file used by SVM_AT_PEA_135.py.
SEX_CSV_PATH = "ICA_Pearson_Full_Features.csv"

# Input .txt ICA timeseries files are provided as positional CLI arguments
# when scheduled via Swiss Army Knife, e.g.:
#   python FINAL_ICAgraphAttention_sex.py /path/to/*.txt
# The first 7 characters of each filename are the 7-digit EID.
# ---------------------------------------------------------------------------
...
# =============================================================================
# LABEL LOADING  (sex_31: 0 = female, 1 = male)
# =============================================================================

LABEL_TO_INDEX = {0: 0, 1: 1}   # sex_31 values map directly to class indices
...
def load_sex_labels(sex_csv_path: str) -> Dict[str, int]:
    ...
    Reads only EID and sex_31 columns so the function works with any
    variant of the Atlas/phenotype CSV (extra columns are ignored).
    sex_31 values: 0 = Female, 1 = Male (UK Biobank encoding).
    """
    df = pd.read_csv(sex_csv_path, usecols=lambda c: c in {"EID", "eid", "sex_31"})
    # Normalise column name to "EID"
...
def build_records_from_files(
    ...
    Returns (retained_records, skipped_paths).
    Files whose EID is not present in sex_labels are skipped and logged.
    """
    records: List[SubjectRecord] = []
    skipped: List[str] = []
    for fpath in sorted(txt_files):
        stem = Path(fpath).stem
        eid = stem[:7]  # first 7 digits = EID
...
class SubjectGraphDataset(Dataset):
    def __init__(
        ...
        train_slice_length: int | None = None,
    ):
        ...
        self.train_slice_length = train_slice_length
    ...
    def __getitem__(self, index: int) -> Dict[str, object]:
        ...
        if self.train_slice_length is not None:
            T = timeseries_np.shape[0]
            max_start = T - self.train_slice_length
            if max_start > 0:
                start = random.randint(0, max_start)
                timeseries_np = timeseries_np[start: start + self.train_slice_length]
...
def collate_subject_batch(batch: Sequence[Dict[str, object]]) -> Dict[str, object]:
    ...
    return {
        ...
        "timeseries": torch.stack([s["timeseries"] for s in batch], dim=1),
...
class STAGIN(nn.Module):
    ...
    def forward(
        ...
        for layer_index, (gin, readout, temporal_block, linear) in enumerate(
            ...
            graph_features, node_attention = readout(h_bridge)
            ...
            pooled = self._pool_time(temporal_output, layer_index)
...
def _compute_metrics(
    ...
    try:
        auroc = float(roc_auc_score(all_labels, all_prob_male))
    except ValueError:
        auroc = float("nan")
    ...
    miss_female = 100.0 * fp / (tn + fp) if (tn + fp) > 0 else float("nan")
    miss_male = 100.0 * fn / (tp + fn) if (tp + fn) > 0 else float("nan")
    ...
    return {
        ...
        "auroc": auroc,
        "f1_male": f1,
        "sensitivity_male": sensitivity,
        "specificity_female": specificity,
        "misclassification_percent_Female": miss_female,
        "misclassification_percent_Male": miss_male,
...
def evaluate_model(
    ...
    for batch in loader:
        ...
        prob_male_np = probabilities[:, 1].cpu().numpy()

        all_labels.extend(labels_np.tolist())
        all_pred_labels.extend(pred_np.tolist())
        all_prob_male.extend(prob_male_np.tolist())

        if collect_attention:
            node_batch = output.node_attention.cpu()
            time_batch = output.time_attention.cpu()
        ...
        for subject_id, true_label, pred_label, prob_male, class_name, csv_path in zip(
            batch["subject_ids"], labels_np.tolist(), pred_np.tolist(),
            prob_male_np.tolist(), batch["class_names"], batch["csv_paths"],
        ):
            prediction_rows.append({
                ...
                "probability_Female": 1.0 - prob_male,
                "correct": int(true_label == pred_label),
                "source_class": class_name,
                "csv_path": csv_path,
            })

    metrics = _compute_metrics(all_labels, all_pred_labels, all_prob_male, total_loss, total_subjects)
...
def main() -> None:
    ...
    # -------------------------------------------------------------------------
    # 1. Load sex labels from the phenotype CSV
    # -------------------------------------------------------------------------
    print(f"Loading sex labels from: {SEX_CSV_PATH}")
    sex_labels = load_sex_labels(SEX_CSV_PATH)
    print(f"  {len(sex_labels)} subjects with sex_31 labels loaded.")

    # -------------------------------------------------------------------------
    # 2. Collect input .txt files
    #    Swiss Army Knife passes all selected files as positional CLI arguments.
    #    Fall back to a glob of the current directory if none are supplied.
    # -------------------------------------------------------------------------
    if len(sys.argv) > 1:
    ...
    print(f"  {len(txt_files)} .txt ICA timeseries files supplied.")
    ...
    with open(skipped_log, "w", encoding="utf-8") as f:
        ...
        f.writelines(f"{s}\n" for s in skipped)
    print(f"  {len(records)} files matched to sex labels; {len(skipped)} skipped.")
    ...
    print(f"  {len(records)} subjects retained after shape screening.")
    ...
    for fold_idx, (train_indices, test_indices) in enumerate(
        ...
        train_loader = DataLoader(
            SubjectGraphDataset(
                train_records, **base_dataset_kwargs,
                train_slice_length=TRAIN_SLICE_LENGTH,
            ),
        ...
        test_loader = DataLoader(
            SubjectGraphDataset(
                test_records, **base_dataset_kwargs,
                train_slice_length=None,
            ),
        ...
        for epoch in range(1, EPOCHS + 1):
            train_loss = run_one_epoch(model, train_loader, optimizer, scheduler, REG_LAMBDA, DEVICE)
            row = {"fold": fold_idx, "epoch": epoch, "train_loss": train_loss}
            fold_history.append(row)
            print(f"  Fold {fold_idx} | Epoch {epoch:03d} | train_loss={train_loss:.4f}")
        ...
        save_metrics_json(
            {"fold": fold_idx, "final_epoch": EPOCHS, "test_metrics": test_metrics},
        ...
        fold_metrics_list.append({
            ...
            "f1_male": test_metrics["f1_male"],
            "sensitivity_male": test_metrics["sensitivity_male"],
            "specificity_female": test_metrics["specificity_female"],
            "misclassification_percent_Female": test_metrics["misclassification_percent_Female"],
            "misclassification_percent_Male": test_metrics["misclassification_percent_Male"],
        })
    ...
    f1_m,  f1_s  = mean_std("f1_male")
    sen_m, sen_s = mean_std("sensitivity_male")
    spe_m, spe_s = mean_std("specificity_female")
    mfe_m, mfe_s = mean_std("misclassification_percent_Female")
    mma_m, mma_s = mean_std("misclassification_percent_Male")

    summary = {
        "accuracy":             {"mean": acc_m, "std": acc_s},
        "auroc":                {"mean": auc_m, "std": auc_s},
        "f1_male":              {"mean": f1_m,  "std": f1_s},
        "sensitivity_male":     {"mean": sen_m, "std": sen_s},
        "specificity_female":   {"mean": spe_m, "std": spe_s},
        "misclassification_Female": {"mean": mfe_m, "std": mfe_s},
        "misclassification_Male":   {"mean": mma_m, "std": mma_s},
        "per_fold": fold_metrics_list,
    }
    save_metrics_json(summary, OUTPUT_DIR / "cv_summary.json")

    print(f"\n{'='*60}\n 5-FOLD CV RESULTS (Binary Sex Classification)\n{'='*60}")
    print(f"  Accuracy      : {acc_m*100:.2f} ± {acc_s*100:.2f}%")
    print(f"  AUROC         : {auc_m:.4f} ± {auc_s:.4f}")
    print(f"  F1 (Male)     : {f1_m:.4f} ± {f1_s:.4f}")
    print(f"  Sensitivity   : {sen_m:.4f} ± {sen_s:.4f}")
    print(f"  Specificity   : {spe_m:.4f} ± {spe_s:.4f}")
    print(f"  Miss % Female : {mfe_m:.2f} ± {mfe_s:.2f}%")
    print(f"  Miss % Male   : {mma_m:.2f} ± {mma_s:.2f}%")
