...
("assembly A"), it:
  ...
  4. Runs a 10,000-permutation bootstrap test to build the null SI distribution:
     ...
     - Significance threshold: 99th percentile of null distribution (p = 0.01),
       exactly as in Almeida et al. (2014).
...
USAGE
-----
  python similarity_index_analysis.py <subject_file.txt> \
         --large_bin <N_TRs> --small_bin <M_TRs> [--subject_id LABEL]

  Example:
    python similarity_index_analysis.py 1234567.txt --large_bin 20 --small_bin 10

  If no arguments are given, the script looks for a single .txt file in the
  current directory and uses default large/small bin sizes from CONFIG.
"""
...
DEFAULT_LARGE_BIN_TRS = 60   # coarser resolution (fewer assemblies expected)
DEFAULT_SMALL_BIN_TRS = 10   # finer resolution (more assemblies expected)

N_PERMUTATIONS   = 10_000    # bootstrap iterations for null SI distribution
SI_ALPHA         = 0.01      # significance level (99th percentile)

OUTPUT_DIR = Path("/Users/simaldolek/Desktop/RESULTS/SEX_CONTROL/bin_size_analysis/ICA/")
