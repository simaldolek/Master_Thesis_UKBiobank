...
def load_cov_stack_from_folder(
    cov_dir: Path,
    pattern: str = "*.npy",
    require_shape: Tuple[int, int] = (414,414), # for Glasser + Tian Subcortex merged
...
def _precision_from_cov_cholesky(Sigma: np.ndarray, jitter: float = 0.0) -> np.ndarray:
    """
    Precision Ω = Σ^{-1} using Cholesky. Mirrors Eq (2) in Pervaiz:
...
def precision_to_partial_corr(Omega: np.ndarray, eps: float = 1e-15) -> np.ndarray:
    ...
    w = np.maximum(w, eps)  # to avoid numerical issues
    denom = np.sqrt(np.outer(w, w))
    rho = -Omega / denom
    np.fill_diagonal(rho, 1.0)
    rho = 0.5 * (rho + rho.T)     # enforce symmetry
    return rho
...
def choose_alpha_pervaiz_exact(
    ...
    # unregularized precisions Ω_b for all subjects
...
def compute_and_save_partial_corrs_pervaiz(
    ...
    n, d, _ = cov_stack.shape
    print(f"Loaded {n} COV matrices of shape {d}x{d} from {cov_dir}")
    ...
    print(f"Global alpha* = {alpha_star:.6g}")
...
cov_dir = "/opt/notebooks/Cov_Matrices/Atlas/PTSD"
out_unreg = "/opt/notebooks/Partial_Corr_Matrices/Atlas/PTSD/Unregularized"
out_tikh  = "/opt/notebooks/Partial_Corr_Matrices/Atlas/PTSD/Tikhonov"
...
alpha_star, diag = compute_and_save_partial_corrs_pervaiz(
    ...
    alpha_grid=np.logspace(-5, 1, 30),
    jitter=1e-8,
    require_shape=(414, 414),
    save_precision_too=False,  # set True if you also want Ω outputs
)
...
# bash
#for f in /opt/notebooks/Partial_Corr_Matrices/Atlas/HC/Unregularized/*.npy; do
#  base=$(basename "$f")
#  dx upload "$f" --path "/Partial_Corr_Matrices/ICA/Unregularized/HC/$base" 
#done
