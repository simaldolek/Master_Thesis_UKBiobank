...
# Kim et al. random time-slice augmentation during training.
# The full timeseries (TIMEPOINTS=490) is used at test time.
# During training a random contiguous slice of this length is drawn.
# Kim et al. used 600 / 1200 for their 1200-timepoint HCP data;
# we keep the same *proportion* scaled to our 490-timepoint data.
# Set to None to disable (uses full series). Adjust as needed.
TRAIN_SLICE_LENGTH = 300   # ~490 * (600/1200) ≈ 245; set higher if GPU allows
...
class SubjectGraphDataset(Dataset):
    ...
    When train_slice_length is set (training mode, following Kim et al.),
    a random contiguous slice of that length is drawn from the full timeseries
    at each __getitem__ call, providing stochastic augmentation.
    At test time train_slice_length=None so the full timeseries is used.
    """
    ...
    def __init__(
        ...
        train_slice_length: int | None = None,  # None → use full series (test/inference)
    ):
        ...
        # Pre-compute endpoints for the *full* series (used at test time / for collation)
    ...
    def __getitem__(self, index: int) -> Dict[str, object]:
        ...
        # -----------------------------------------------------------------
        # Kim et al. random time-slice augmentation (training only)
        # "The time dimension of ROI-timeseries matrix P was randomly sliced
        #  with a fixed length [600] at each step during training for
        #  (i) relieving computational overload,
        #  (ii) stochastic augmentation of the training dataset."
        # Full unsliced matrix is used for inference at test time.
        # -----------------------------------------------------------------
        if self.train_slice_length is not None:
            ...
            # If T <= train_slice_length, use full series (no slicing needed)
...
def main() -> None:
    ...
    # 5-fold stratified CV — identical to Kim et al.
    skf = StratifiedKFold(n_splits=5, shuffle=True, random_state=SEED)

    # Shared kwargs for both train and test datasets.
    # train_slice_length is set separately for each split below.
    ...
    for fold_idx, (train_indices, test_indices) in enumerate(
        ...
        # All non-test subjects are used for training — no held-out val split.
        ...
        # Train loader: random time-slice augmentation enabled (Kim et al.)
        train_loader = DataLoader(
            SubjectGraphDataset(
                train_records,
                **base_dataset_kwargs,
                train_slice_length=TRAIN_SLICE_LENGTH,  # stochastic slice each epoch
            ),
            batch_size=BATCH_SIZE, shuffle=True, collate_fn=collate_subject_batch,
        )

        # Test loader: full unsliced timeseries (Kim et al. inference setting)
        test_loader = DataLoader(
            SubjectGraphDataset(
                test_records,
                **base_dataset_kwargs,
                train_slice_length=None,  # full series at inference
            ),
        ...
        scheduler = torch.optim.lr_scheduler.OneCycleLR(
            ...
            final_div_factor=2000,  # decays to 1e-3 / 2000 = 5e-7
        )

        fold_history: List[Dict] = []

        # -----------------------------------------------------------------
        # Train for exactly EPOCHS epochs — no early stopping, no checkpoint
        # selection on the test fold (Kim et al. fixed-epoch protocol).
        # -----------------------------------------------------------------
        for epoch in range(1, EPOCHS + 1):
            ...
            print(
                f"  Fold {fold_idx} | Epoch {epoch:03d} | train_loss={train_loss:.4f}"
            )
        ...
        # -----------------------------------------------------------------
        # Final evaluation on the LAST-epoch model (Kim et al. protocol).
        # No checkpoint reloading — we simply evaluate the model as-is after
        # training completes.
        # -----------------------------------------------------------------
        ...
        # Save final model weights for reproducibility / analysis
        torch.save(
            {
