...
"""
Improved version of fullGraphAttention_v1.py. What's NEW:
...
class STAGIN(nn.Module):
    ...
    def __init__(
        ...
        self.pre_classifier_dropout = nn.Dropout(dropout + 0.1)  #   NEW : slightly higher 
    ...
    def forward(
        ...
        logits = self.classifier(self.pre_classifier_dropout(final_representation))
...
def main() -> None:
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau( # added scheduler
    optimizer,
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        scheduler.step(val_metrics["auroc"]) # NEW
      
        if early_stopper.step(val_metrics["auroc"]): # NEW
