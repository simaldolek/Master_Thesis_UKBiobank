...
"""
v3 with 5 fold cross validation and saved attention weights
"""
...
class GINLayer(nn.Module):
    """Sparse GIN update matching original STAGIN: operates on flattened (B*W*N, D) tensors."""

    def __init__(self, feature_dim: int):
        super().__init__()
        self.eps = nn.Parameter(torch.zeros(1, 1))  # shape [1,1] matches original
    ...
    def forward(self, v: torch.Tensor, a: torch.Tensor) -> torch.Tensor:
        # v: [(B*W*N), D],  a: sparse [(B*W*N), (B*W*N)]
        v_aggregate = torch.sparse.mm(a, v)
        v_aggregate += self.eps * v
        return self.mlp(v_aggregate)
...
class STAGIN(nn.Module):
    ...
    def _collate_adjacency(self, a: torch.Tensor) -> torch.Tensor:
        """Convert dense [B, W, N, N] adjacency to a sparse [(B*W*N), (B*W*N)] matrix."""
        i_list, v_list = [], []
    ...
    def forward(
        ...
        # --- Initial projection, flatten to [(B*W*N), D] ---
        ...
        for layer_index, (gin, readout, temporal_block, linear) in enumerate(
            ...
            # GIN on flattened sparse tensor
            h = gin(h, sparse_a)

            # Reshape back to [B, W, N, D] for readout
            h_bridge = h.reshape(batch_size, num_windows, num_nodes, self.hidden_dim)

            # Spatial readout → [B, W, D]
            ...
            temporal_output, time_attention = temporal_block(temporal_input)  # [W, B, D]

            # Pool time → [B, D]
            pooled = self._pool_time(temporal_output, layer_index)

            # *** Original STAGIN: accumulate per-layer logits ***
            logit = logit + self.dropout(linear(pooled))

            # Orthogonality penalty — row-wise max normalization (original)
        ...
        node_attention_out = torch.stack(per_layer_node_attention, dim=1)
...
def evaluate_model(
    ...
    all_node_attention: List[torch.Tensor] = []   # added
    all_time_attention: List[torch.Tensor] = []   # added
    ...
    for batch in loader:
        ...
        all_node_attention.append(output.node_attention.cpu())  # added  [B, K, W, N]
        all_time_attention.append(output.time_attention.cpu())  # added  [B, K, W, W]
    ...
    node_attn_tensor = torch.cat(all_node_attention, dim=0)  # added  [N_subjects, K, W, N]
    time_attn_tensor = torch.cat(all_time_attention, dim=0)  # added  [N_subjects, K, W, W]

    predictions_df = pd.DataFrame(prediction_rows)
    predictions_df["node_attention_mean"] = node_attn_tensor.mean(dim=(1, 2)).tolist()  # per-subject mean across layers+windows
    
    return metrics, predictions_df, node_attn_tensor, time_attn_tensor  
...
def main() -> None:
    ...
    # -------------------------------------------------------------------------
    # 5-fold stratified cross-validation (matches paper exactly)
    # -------------------------------------------------------------------------
    ...
    for fold_idx, (train_indices, test_indices) in enumerate(
        ...
        save_records_csv(train_records, fold_output_dir / "train_split.csv")
        save_records_csv(test_records,  fold_output_dir / "test_split.csv")
        ...
        for epoch in range(1, EPOCHS + 1):
            ...
            # Paper evaluates on the held-out fold after each epoch
            ...
            fold_history.append(epoch_summary)
        ...
        # Load best checkpoint for this fold's final evaluation
        checkpoint = torch.load(best_model_path, map_location=DEVICE, weights_only=False)
        model.load_state_dict(checkpoint["model_state_dict"])

        test_metrics, test_predictions,node_attn, time_attn = evaluate_model(model, test_loader, REG_LAMBDA, DEVICE)
        test_predictions["fold"] = fold_idx
        all_fold_predictions.append(test_predictions)

        torch.save(node_attn, fold_output_dir / "node_attention.pt")  
        torch.save(time_attn, fold_output_dir / "time_attention.pt")  
      
        save_metrics_json(
            {"fold": fold_idx, "best_val_auroc": best_fold_auroc,
             "test_metrics": test_metrics, "history": fold_history},
            fold_output_dir / "metrics.json",
        )
    ...
    # -------------------------------------------------------------------------
    # Aggregate across folds (mean ± std, matching paper's reporting format)
    # -------------------------------------------------------------------------
    ...
    summary = {
        ...
        "auroc_std":      float(metrics_df["auroc"].std()),
        "misclassification_percent_HC_mean":   float(metrics_df["misclassification_percent_HC"].mean()),
        "misclassification_percent_PTSD_mean": float(metrics_df["misclassification_percent_PTSD"].mean()),
    ...
    print(f"  Miss % HC  : {summary['misclassification_percent_HC_mean']:.2f}%")
    print(f"  Miss % PTSD: {summary['misclassification_percent_PTSD_mean']:.2f}%")
